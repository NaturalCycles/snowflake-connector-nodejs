// @smithy/node-http-handler is an optional peerDependency; only required when a proxy is configured.
const fs = require('fs');
 ...
 * @constructor
 */
function S3Util(connectionConfig, s3) {
  ...
  this.uploadFile = async function (dataFile, meta, encryptionMetadata) {
    if (GlobalConfigTyped.getValue('enableExperimentalMultipartUploads')) {
      ...
      if (fileSize > MULTIPART_THRESHOLD_BYTES) {
        ...
        // TODO: temporarily use readFile instead of createReadStream to work around a Bun bug
        // where createReadStream fails the upload in compiled apps. Investigate the root cause
        // and fix it properly.
  ...
  this.uploadFileMultipart = async function (dataFile, fileSize, meta, encryptionMetadata) {
    ...
    let uploadId;
    try {
      const createResp = await client.createMultipartUpload({
        Bucket,
        Key,
        Metadata: buildS3Metadata(meta, encryptionMetadata),
      });
      ...
      try {
        ...
        while (position < fileSize) {
          ...
          const partResp = await client.uploadPart({
            Bucket,
            Key,
            UploadId: uploadId,
  ...
  this.uploadFileStream = async function (fileStream, meta, encryptionMetadata) {
    const s3Metadata = buildS3Metadata(meta, encryptionMetadata);
...
/**
 * Build the S3 object metadata, including the encryption envelope when the
 * upload is encrypted, so every upload path persists the same metadata shape.
 */
function buildS3Metadata(meta, encryptionMetadata) {
  const s3Metadata = {
...
/**
 * Classify an upload failure into the file-transfer-agent's `meta.resultStatus`
 * convention so retry semantics stay consistent across every catch site. The
 * error may be an S3 SDK error (mapped to RENEW_TOKEN /
 * NEED_RETRY_WITH_LOWER_CONCURRENCY) or any other JS error (e.g. a short read),
 * which falls back to NEED_RETRY.
 */
